...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  ...
  let sumMessage = `${a} and ${b} and ${c} sum to ${sumTwo}.`;
  let multiplyMessage = `The product of ${a} and ${b} and ${c} is ${multiplyTwo}.`
  let sumAndMultiplyArr = [sumTwo, multiplyTwo, sumMessage, multiplyMessage];
...
function multiplyArray(Arr) { 
  let product = multiply(multiply(Arr[0], Arr[1])[0], Arr[2])[0];
  let message = `The numbers ${Arr[0]},${Arr[1]},${Arr[2]} have a product of ${product}.`;
...
function multiplyAnyArray(Arr) { 
  let product = multiply(Arr[0], Arr[1])[0];
  if (Arr.length > 2) {
    for (let i = 2; i < Arr.length; i++) {
      product = multiply(product, Arr[i])[0];
    }
  }
  let message = `The numbers ${Arr} have a product of ${product}.`;
  return [product,message];
  //eslint-disable-line
